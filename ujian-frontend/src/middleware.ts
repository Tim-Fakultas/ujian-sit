...
export function middleware(request: NextRequest) {
  ...
  if (!authCookie && (pathname.startsWith('/admin') || pathname.startsWith('/dosen') || pathname.startsWith('/mahasiswa'))) {
  ...
  if (userCookie) {
    try {
      ...
      const userRole = userData.roles && userData.roles.length > 0 ? userData.roles[0].name : null;
