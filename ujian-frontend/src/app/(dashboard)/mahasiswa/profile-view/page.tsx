...
async function getStudent(nim: string): Promise<Student> {
  const res = await fetch(`http://127.0.0.1:8000/api/mahasiswa/${nim}`, {
    cache: "no-store", // biar selalu fresh
  });

  if (!res.ok) {
    throw new Error("Gagal fetch data mahasiswa");
  }
...
export default async function StudentProfilePage({
  ...
  const studentData = await getStudent(params.nim);
